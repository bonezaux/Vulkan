...
inline void dump_vkGetBufferDeviceAddressEXT(ApiDumpInstance& dump_inst, VkDeviceAddress result, VkDevice device, const VkBufferDeviceAddressInfoEXT* pInfo)
{
    loader_platform_thread_lock_mutex(dump_inst.outputMutex());
    switch(dump_inst.settings().format())
    {
    case ApiDumpFormat::Text:
        dump_text_vkGetBufferDeviceAddressEXT(dump_inst, result, device, pInfo);
        break;
    case ApiDumpFormat::Html:
        dump_html_vkGetBufferDeviceAddressEXT(dump_inst, result, device, pInfo);
        break;
    }
    loader_platform_thread_unlock_mutex(dump_inst.outputMutex());
}
...
VK_LAYER_EXPORT VKAPI_ATTR VkDeviceAddress VKAPI_CALL vkGetBufferDeviceAddressEXT(VkDevice device, const VkBufferDeviceAddressInfoEXT* pInfo)
{
    ...
    dump_vkGetBufferDeviceAddressEXT(ApiDumpInstance::current(), result, device, pInfo);
    return result;
}
